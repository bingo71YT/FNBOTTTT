...
for (var i = 0; i < 1e7; i++) {
  if ((new Date().getTime() - start) > milliseconds){
	...
	partyBuildId: '1:1:' + config.netcl,
      ...
      fortnite.communicator.on('party:member:joined', async (member) => {
        ...
        console.log(`Members count: ${fortnite.party.members.length}`);
          ...
          fortnite.party.me.setBattlePass(true, -810, -0, -114514810);
      ...
      fortnite.communicator.off('friend:request', async (friendops) => {
      ...
      eg.communicator.on('friend:message', async (data) => {
        ...
        if(args[0].includes('CID_')){
          ...
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "スキンを" + args[0] + "にセット");
          } catch {
        ...
        if(args[0].includes('EID_')){
          ...
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "エモートを" + args[0] + "にセット");
          } catch {
        ...
        if(args[0].includes('Pickaxe_ID_')){
          ...
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "ピッケルを" + args[0] + "にセット");
          } catch {
        ...
        if(args[0].includes('BID_')){
          ...
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "バッグブリングを" + args[0] + "にセット");
          } catch {
    ...
    if (args[0].toLowerCase() == "!update"){
      ...
      fortnite.party.me.setEmote("/Game/Athena/Items/Cosmetics/Dances/" + config.eid + "." + config.eid);
        ...
        if(args[0].toLowerCase() == "!ready"){
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "準備OK!");
          }
        }
        ...
        if(args[0].toLowerCase() == "!unready"){
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "準備中");
          }
        }
        ...
        if(args[0].toLowerCase() == "!bp"){
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "バトルパスを" + args[1] + " " + args[2] + " " + args[3] + "にセット");
          } catch {
        ...
        if(args[0].toLowerCase() == "!stop"){
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "エモートストップ...");
          } catch {
        ...
        if(args[0].toLowerCase() == "!platform"){
          try {
            ...
            eg.communicator.sendMessage(data.friend.id, "プラットフォームを " + args[1] + " にセット!");
          } catch {
      ...
      fortnite.communicator.updateStatus('死ね');
    });
